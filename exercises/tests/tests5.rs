...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The `address` is guaranteed to be a valid mutable reference
    // to a `u32` value, as specified by the function's contract. We write
    // the expected value through that pointer.
    unsafe {
